...
void Player::Draw() 
{
	...
	glPopMatrix();
		...
		if(m_attackState)	{
			...
			sprintf(temp, "Attack! %d", m_atkDir);
			drawStrokeText(temp, 0, 20, 0, 0.05);
		}
}
...
void Player::SetCameraViewer(_point* cameraPosition, _point* cameraCenter)
{
	/*_point resultPosition;

	resultPosition.x = point.x + m_velocity.x;
	resultPosition.y = point.y + m_velocity.y;
	resultPosition.z = point.z + m_velocity.z;*/

	float scalar = sqrtf(pow(m_velocity.x, 2) + pow(m_velocity.y, 2) + pow(m_velocity.z, 2));

	_point unitVector;

	unitVector.x = m_velocity.x / scalar;
	unitVector.y = m_velocity.y / scalar;
	unitVector.z = m_velocity.z / scalar;

	cameraPosition->x = point.x;
	cameraPosition->y = point.y;
	cameraPosition->z = point.z;

	/*cameraCenter->x = point.x + unitVector.x;
	cameraCenter->y = point.y + unitVector.y;
	cameraCenter->z = point.z + unitVector.z;*/
