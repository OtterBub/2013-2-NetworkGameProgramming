...
_point g_camera_eye = {0, 150, 80};
_point g_camera_center = {0, 0, -1};

double g_eye_x = 0, g_eye_y = 150, g_eye_z = 80;
...
GLvoid drawScene ( GLvoid )
{
	...
	//g_camera_center = myChar->GetPoint();

	myChar->SetCameraViewer(&g_camera_eye, &g_camera_center);
	...
	// Camera Control
	/*gluLookAt(g_camera_eye.x + g_camera_center.x, g_camera_eye.y + g_camera_center.y, g_camera_eye.z + g_camera_center.z,
		g_camera_center.x, g_camera_center.y, g_camera_center.z,
		0.0, 1.0, 0.0);*/

	gluLookAt(g_camera_eye.x, g_camera_eye.y, g_camera_eye.z,
		g_camera_center.x, g_camera_center.y, g_camera_center.z,
		0.0, 1.0, 0.0);
...
GLvoid MenuFuntion(int value)
{
	switch(value){
		case 1111:
		{
			...
			glOrtho(-(g_w/6), (g_w/6), -(g_h/6), (g_h/6), -10, 2000);
