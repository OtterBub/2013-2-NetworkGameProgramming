...
void Monster::Update() {
	...
	WorldData TempWorldData = RPGWorld::Instance()->GetWorldData();

	int ClosestUserDist = INT_MAX;
	_point ClosestUserPoint = {0, };
	for(int i =  0; i < TempWorldData.PlayerNum; i++) {
		_point userPoint;
		userPoint = TempWorldData.playerInfo[i].UserPosition;

		_point distance;
		distance = GetDist(userPoint);
		float Dist = abs(distance.x) + abs(distance.y) + abs(distance.z);
		if(Dist < ClosestUserDist) {
			ClosestUserDist = Dist;
			ClosestUserPoint = userPoint;
		}
		
	}

	if(ClosestUserDist < 50) {
		m_state = MOB_SEEK;
		if(m_hp <= 30) {
			m_state = MOB_FLEE;
		}
	...
	case MonsterState::MOB_SEEK :
		ObjectSeek(ClosestUserPoint, 0.2);
		break;

	case MonsterState::MOB_FLEE :
		ObjectFlee(ClosestUserPoint, 0.2);
		break;
